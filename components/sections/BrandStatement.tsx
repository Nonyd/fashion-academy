...
export default function BrandStatement() {
  return (
    ...
    >
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col gap-16 lg:flex-row lg:items-center lg:justify-between lg:gap-20">
        <motion.blockquote
          initial={{ opacity: 0, y: 40 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.5 }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="relative max-w-2xl text-3xl leading-[1.15] sm:text-4xl md:text-[3.3rem] lg:max-w-xl"
        >
        ...
        <motion.dl
          ...
          className="grid flex-1 grid-cols-1 gap-10 sm:grid-cols-3 sm:gap-x-16 sm:gap-y-10 md:gap-x-20 lg:gap-x-24 lg:border-l-2 lg:border-[var(--color-noir)] lg:pl-12"
        >
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="flex min-w-0 flex-col gap-2 px-1 text-left sm:min-w-[8rem] sm:px-0 sm:text-left lg:text-left"
            >
              <dt className="text-[10px] font-medium uppercase tracking-[0.28em] text-[var(--color-muted)]">
                {stat.label}
              </dt>
              <dd className="text-2xl font-bold tracking-tight text-[var(--color-noir)] sm:text-3xl">
...
function AnimatedCounter({ displayValue }: AnimatedCounterProps) {
  ...
  const suffix = displayValue.replace(/^[\d,\s]+/, "") || "";
